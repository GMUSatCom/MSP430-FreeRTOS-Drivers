...
void SatLib::initSatLib()
{
    ...
    for(channel = 0; channel < __MSP430_HAS_DMA__; channel++)
    {
        ...
        DMA_Channels[channel].finished = xSemaphoreCreateBinaryStatic(DMA_Channels[channel].pxFinishedBuffer);
        xSemaphoreGive(DMA_Channels[channel].mutex);
        xSemaphoreGive(DMA_Channels[channel].finished);
    }
